...
//
// Data probe (2026-06-07 controller/I-O decomposition pass):
//   - The focal product has exactly 78 `requires`-reachable structural
...
//     MUST be excluded.
test("focusId=focal product: includes 78-node subtree; iPhone product + iPhone-only material excluded", () => {
  const graph = loadGraphData();
  ...
  // Size is pinned: oracle === function-under-test === 78.
  assert.equal(
    expectedSubtree.size,
    78,
    `oracle: focal subtree should be 78; got ${expectedSubtree.size}. ` +
      "If this fails, data changed — adjust the assertion and re-pin the spec.",
  );
  assert.equal(
    subset.nodes.size,
    78,
    `focusedSubset must include exactly the 78 requires-descendants of focal; got ${subset.nodes.size}`,
  );
