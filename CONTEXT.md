...
- **Metrics** (cost, throughput, accuracy, range, etc.) are first-class `metric`-kind nodes attached via `measured_by` edges. Data-layer they are separate nodes; display-layer they fold into the parent node's detail panel as text and are never rendered on canvas (per ADR-0006, 2026-05-13).
