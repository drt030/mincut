...
function topReadiness(graph: GraphData): TopNodeReadiness[] {
  return selectTopN(graph, "bottleneck-risk", 3, null).map((entry) => {
    ...
    return {
      ...
      independentEvidence: directIndependentEvidence(graph, node),
...
function formatTopNode(entry: TopNodeReadiness): string {
  const gaps = [
    ...
    entry.independentEvidence > 0 ? null : "independentEvidence",
...
for (const domain of DOMAIN_ROUTES) {
  ...
  const auditPreviewTopGapCount = top.reduce((count, entry) => {
    const ready =
      entry.costAnswer &&
      entry.leadTime &&
      entry.constraint &&
      entry.independentEvidence > 0;
    return count + (ready ? 0 : 1);
  }, 0);
  const paidTopGapCount = top.reduce((count, entry) => {
    const ready =
      entry.costAnswer &&
      entry.leadTime &&
      entry.constraint &&
  ...
  console.log(
    [
      ...
      `auditPreviewTopGaps=${auditPreviewTopGapCount}/${top.length}`,
      `paidTopGaps=${paidTopGapCount}/${top.length}`,
      `top=${top.map(formatTopNode).join(", ")}`,
    ].join(" "),
  );

  if (auditPreviewStates.has(domain.portfolioState)) {
    for (const entry of top) {
      if (!entry.costAnswer) failures.push(`${domain.slug}/${entry.id} missing cost answer for audit-preview.`);
      if (!entry.leadTime) {
        failures.push(`${domain.slug}/${entry.id} missing capacityLeadTimeMonths for audit-preview.`);
      }
      if (!entry.constraint) failures.push(`${domain.slug}/${entry.id} missing constraint_* tag for audit-preview.`);
      if (entry.independentEvidence === 0) {
        failures.push(`${domain.slug}/${entry.id} missing independent non-vendor evidence for audit-preview.`);
      }
    }
  }
