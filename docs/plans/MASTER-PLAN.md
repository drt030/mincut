...
- - **Risk reversal (locked 2026-06-11, pre-launch):** price stays $9/$29 — at zero brand trust the binding friction is trust+card-entry, not the $4 delta (a $4.99 cut needs +69% conversion to break even and muddies the pre-registered gates). Instead, the lock CTA + checkout carry a 7-day no-questions refund line ("reply to your receipt — full refund, keep access"). Cheaper-price probe ($4.99) remains exactly where it belongs: the PIVOT-PACKAGING branch, decided on Sunday's funnel data, not pre-launch nerves. Price-sensitive segment is served by the Tue referral unlock (effort currency).
