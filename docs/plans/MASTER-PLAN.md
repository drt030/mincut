...
| **4. Product/know-how layer split** (worktree `worktree-product-knowhow-layers`) | [spec](../superpowers/specs/2026-06-10-product-knowhow-layers-design.md) · [impl plan](../superpowers/plans/2026-06-10-product-knowhow-layers.md) (12 TDD tasks, ~1.5–2 days) | **READY TO MERGE (2026-06-11 ~01:30)** — 17 commits at `3e47687`: all 12 plan tasks ✅ + Decision-10 teaser integration ✅ (`computeHolderTeasers` runs server-side on the pre-strip graph → context provider; org identities stay behind `stripExposureLayer`; locked domains keep true holder counts; compatible with the 4221f02 fail-closed gate + free_teaser pass-through). Rebased onto master three times (7c1ba6f → 28e5ac7 → 5b666ed). verify: lint ✓ ux-checks ✓ tests 328/329 ✓ build ✓ — the 1 test fail (gatePage ordering), validate-data exit-1 (8 stale task↔gate-report refs), and the /graph default-root hydration hang all reproduce on bare master 28e5ac7 (lane-1 uncommitted WIP appears to fix them). Visual acceptance on :3100 with `?root=parcel` ✓: 54 artifact nodes position-stable across layer flip, 30 transactability-colored diamonds, red-ring bottleneck badges, holder/listed teaser in rail. | **`git merge --ff-only worktree-product-knowhow-layers`** — ff currently refused only because lane-1's uncommitted `GraphExplorer.tsx` + `graphTraversal.ts` edits would be overwritten; merge the moment lane-1 commits. Follow-ups: transactability backfill for 17 ai-chain + 29 test-fixture know-how nodes (warn-only); `ExposureLockCta.tsx` lacks a React import (latent unit-test-harness issue, fine in Next) | Merge gated on lane-1 commit; Thu 22:00 comfortably reachable |
